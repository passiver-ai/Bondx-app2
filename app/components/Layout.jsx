...
const Layout = ({ children, showBottomBar, title, hasBackButton = false }) => {
  ...
  return (
    <div className="h-full w-full ">
      <div
        className={`${
          showBottomBar ? "h-[calc(100vh-76px)]" : "h-[100vh]"
      ...
      >
        {hasBackButton ? (
          <div className="h-[56px] pretendard-500 text-[24px] text-[#1E293B] flex items-center sticky top-0 bg-white px-6 z-[99]">
            ...
            <div className="text-center justify-center items-center absolute left-1/2 -translate-x-1/2">
              {title}
            </div>
          </div>
        ) : (
          <div className="h-[56px] inter-600 text-[24px] flex items-center sticky top-0 bg-white px-6">
            {title}
          </div>
        )}
